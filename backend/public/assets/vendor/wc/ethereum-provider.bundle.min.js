...
<body>
  ...
  document.addEventListener('DOMContentLoaded', ()=> {
    ...
    function loadScript(src){return new Promise((resolve,reject)=>{const s=document.createElement('script');s.src=src;s.async=true;s.onload=resolve;s.onerror=()=>reject(new Error("Échec chargement "+src));document.head.appendChild(s)})}
      ...
      btn.addEventListener('click', async ()=>{
        ...
        if(provider==='metamask'){
          ...
          if(!eth||!eth.request){if(isStandaloneApp()&&!isMobile){window.open(window.location.href,'_blank');return}
            ...
            alert("MetaMask non détecté. Installe l’extension puis recharge la page.");return}
          try{const accounts=await eth.request({method:'eth_requestAccounts'});if(accounts&&accounts[0])walletInput.value=accounts[0];closeModal()}catch(e){console.error(e);alert("Connexion MetaMask refusée ou erreur.")}

        }else if(provider==='phantom'){
          if(window.solana&&window.solana.isPhantom){window.solana.connect().then(r=>{walletInput.value=r.publicKey.toString();closeModal()}).catch(err=>{console.error(err);alert("Connexion Phantom refusée ou erreur.")})}
          else{window.open("https://phantom.app/","_blank")}

        }else if(provider==='uniswap'){
          const uni=getProviderBy(d=>/uniswap/i.test(d?.info?.rdns||'')||/uniswap/i.test(d?.info?.name||''),p=>!!p.isUniswap||p?.providerName==='uniswap');
          if(uni&&uni.request){try{const accounts=await uni.request({method:'eth_requestAccounts'});if(accounts&&accounts[0])walletInput.value=accounts[0];closeModal();return}catch(err){console.error(err);alert("Connexion Uniswap refusée ou erreur.");return}}
          if(isMobile){alert("Sur mobile, la connexion directe au wallet Uniswap n’est pas dispo sans WalletConnect. J’ouvre l’app web.");window.open(`https://app.uniswap.org/#/swap?chain=ethereum`,"_blank","noopener,noreferrer");closeModal();return}
          if(isStandaloneApp()){window.open(window.location.href,'_blank');return}
          alert("Extension Uniswap non détectée. J’ouvre l’app Uniswap dans un nouvel onglet.");window.open(`https://app.uniswap.org/#/swap?chain=ethereum`,"_blank","noopener,noreferrer");closeModal()

        }else if(provider==='walletconnect'){
          try{
            if(window.__wcConnecting)return; window.__wcConnecting=true;
            const EP=await loadWalletConnectProvider(); if(!EP||!EP.init) throw new Error("Provider WalletConnect indisponible (chargement)");
            if(!window.__wcProvider){
              window.__wcProvider=await EP.init({
                ...
                methods:["eth_requestAccounts","eth_sendTransaction","personal_sign","eth_signTypedData","eth_signTypedData_v4","eth_sign"],
                events:["chainChanged","accountsChanged"]
              });
            }
            await window.__wcProvider.connect();
            let accounts=null;
            try{accounts=await window.__wcProvider.request({method:"eth_requestAccounts"})}catch{accounts=await window.__wcProvider.request({method:"eth_accounts"})}
            if(accounts&&accounts[0])walletInput.value=accounts[0];
            closeModal();
          }catch(err){console.error("WalletConnect connect error:",err);alert("Connexion WalletConnect refusée ou erreur.")}
          finally{window.__wcConnecting=false}
        }else{
    ...
    let contacts=JSON.parse(localStorage.getItem("contacts")||"[]");if(showContacts)showContacts.textContent="📖";
    ...
    function renderContacts(){contactsList.innerHTML="";if(contacts.length===0){contactsList.style.display="none";return}
      contacts.forEach((c,idx)=>{const row=document.createElement("div");row.className="contact-item";row.style.display="flex";row.style.alignItems="center";row.style.justifyContent="space-between";row.style.gap="8px";
        ...
        const del=document.createElement("button");del.type="button";del.title=(lang==='fr')?"Supprimer":"Delete";del.textContent="🗑️";del.style.border="1px solid var(--stroke)";del.style.background="#0a1220";del.style.color="#ffffff";del.style.borderRadius="6px";del.style.padding="4px 6px";del.style.cursor="pointer";del.style.flex="0 0 auto";
    ...
    const saveConfirmBtn=document.createElement('button');saveConfirmBtn.type='button';saveConfirmBtn.textContent=`✅ ${t[lang].save}`;saveConfirmBtn.style.border='1px solid var(--stroke)';saveConfirmBtn.style.background='#0a1220';saveConfirmBtn.style.color='#ffffff';saveConfirmBtn.style.borderRadius='8px';saveConfirmBtn.style.padding='6px 10px';saveConfirmBtn.style.cursor='pointer';
    const saveCancelBtn=document.createElement('button');saveCancelBtn.type='button';saveCancelBtn.textContent=`✖️ ${t[lang].cancel}`;saveCancelBtn.style.border='1px solid var(--stroke)';saveCancelBtn.style.background='#0a1220';saveCancelBtn.style.color='#ffffff';saveCancelBtn.style.borderRadius='8px';saveCancelBtn.style.padding='6px 10px';saveCancelBtn.style.cursor='pointer';
    ...
    document.getElementById('generateBtn').addEventListener('click',async()=>{
      ...
      try{
        ...
        const data=await res.json();if(!res.ok)throw new Error(data.detail||"Erreur lors de la génération du lien");
        const lien=`https://linkisend.io/${data.short_id}`;linkEl.textContent=lien;resultDiv.style.display='block';
      }catch(err){errorDiv.textContent=err.message||(lang==='fr'?'Erreur réseau':'Network error');errorDiv.style.display='block'}
    });

    const copyBtn=document.getElementById('copyLinkBtn');if(copyBtn){copyBtn.addEventListener('click',()=>{navigator.clipboard.writeText(linkEl.textContent).then(()=>{copyBtn.textContent="✅";setTimeout(()=>copyBtn.textContent="📋",1500)})})}

    if("serviceWorker" in navigator){navigator.serviceWorker.register("/service-worker.js").then(()=>console.log("SW enregistré ✅")).catch(err=>console.error("SW erreur:",err))}
