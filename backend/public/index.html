...
<body>
  ...
  document.addEventListener('DOMContentLoaded', ()=> {
    ...
    window.addEventListener('eip6963:announceProvider', (event) => {
      try { discovered.push(event.detail); } catch {}
    });
    ...
    function getProviderBy(match6963, matchLegacy){
      const d = discovered.find(x => { try { return match6963(x); } catch { return false; } });
      if (d && d.provider) return d.provider;
      if (window.ethereum) {
        if (Array.isArray(window.ethereum.providers)) {
          const p = window.ethereum.providers.find(p => { try { return matchLegacy(p); } catch { return false; } });
          if (p) return p;
        }
        if (matchLegacy(window.ethereum)) return window.ethereum;
      }
      return null;
    }
    ...
    modal.querySelectorAll('.pv').forEach(btn=>{
      btn.addEventListener('click', async ()=>{
        ...
        if(provider === 'metamask'){
          const eth = getProviderBy(
            d => /metamask/i.test(d?.info?.rdns || '') || /metamask/i.test(d?.info?.name || ''),
            p => !!p.isMetaMask
          );
          if (!eth || !eth.request) {
          ...
          try{
            ...
            closeModal();
          }catch(e){ console.error(e); alert("Connexion MetaMask refusée ou erreur."); }

        } else if (provider === "phantom") {
          if (window.solana && window.solana.isPhantom) {
            window.solana.connect()
              .then(resp => { walletInput.value = resp.publicKey.toString(); closeModal(); })
              .catch(err => { console.error(err); alert("Connexion Phantom refusée ou erreur."); });
          } else {
